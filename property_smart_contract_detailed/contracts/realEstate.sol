...
contract realEstate {
	...
	modifier isMultipleOf{
	   require(msg.value % totalSupply2 == 0);              //modulo operation, only allow ether ammounts that are multiples of totalsupply^2. 
	   	_;													//This is because there is no float and we will divide incoming ammounts two times to split it up and we do not want a remainder.
	}
...
//Stakeholder functions
	...
	function claimOwnership () public {             //claim main property ownership
		require(shares[msg.sender] > (totalSupply /2) && msg.sender != mainPropertyOwner,"Error. You do not own more than 50% of the property tokens or you are the main owner already");
